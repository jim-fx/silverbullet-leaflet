...
loadJsByUrl("https://unpkg.com/leaflet@1.9.4/dist/leaflet.js").then(() => {
  ...
  style.innerHTML =
    ".darkMode .leaflet-layer,.leaflet-control-zoom-in,.leaflet-control-zoom-out,.leaflet-control-attribution {\\n\\tfilter: invert(100%) hue-rotate(180deg) brightness(95%) contrast(90%) !important;\\n}";
    ...
    if (data.markers !== undefined) {
      if (data.markers.length !== 0) {
        data.markers.forEach((markerInfo) => {
          var marker = L.marker([markerInfo.loc[0], markerInfo.loc[1]]).addTo(
            map,
          );

          if (markerInfo.name === undefined) {
            marker.bindPopup(
              "[" + markerInfo.loc[0].toString() + ", " +
                markerInfo.loc[1].toString() + "]",
            );
          } else {
            if (markerInfo.isPageLink === true) {
              ...
              marker.bindPopup(
                '<a href="/' + markerInfo.name + '">' + displayName + "</a>",
              );
            } else {
